...
# Phase 2A baseline = canonical ALL_FEATURES minus the
# three BTC relational experimental features.
BTC_FEATURES = {
    "btc_corr_20",
    "btc_beta_20",
    "btc_rel_strength",
...
def validate_schema(ds: pd.DataFrame) -> None:
    ...
    if set(BASE_FEATURES) - canonical:
        raise ValueError(
            "FATAL: BASE_FEATURES contains non-canonical features."
        )
    ...
    if set(BASE_FEATURES) != expected:
        raise ValueError(
            "FATAL: Phase 2C baseline does not match "
            "the 30-feature Phase 2A baseline."
        )

    if len(BASE_FEATURES) != 30:
        raise ValueError(
            f"FATAL: Expected 30 Phase 2C baseline features, "
    ...
    for name, features in GROUPS.items():
        if not set(features) <= set(BASE_FEATURES):
            raise ValueError(
                f"FATAL: Phase 2C group '{name}' "
                "contains features outside baseline."
            )

    if ds.empty:
        raise ValueError(
            "FATAL: Dataset is empty."
        )
    ...
    missing = sorted(
        required - set(ds.columns)
    )

    if missing:
        raise ValueError(
            f"FATAL: Dataset missing columns: {missing}"
        )
...
def rolling_split(
    ...
    for wno, origin in enumerate(
        ...
        for _, grp in ds.groupby(
            ...
            test_parts.append(
                grp.iloc[
                    test_start:test_start + test_block
                ]
            )
...
def feature_matrix(
    ...
    if missing:
        raise ValueError(
            f"FATAL: Missing features: {missing}"
        )
...
def run_window(
    ...
    if not required <= set(classes):
        raise ValueError(
            "FATAL: Window missing target class: "
            f"{required - set(classes)}"
        )
    ...
    ptest = calibrated.predict_proba(
        Xte[selected].values
    )
...
def main():
    ...
    if len(windows) != 4:
        raise ValueError(
            f"FATAL: Expected 4 rolling windows, "
            f"got {len(windows)}"
        )

    log.info(
        "Using %d chronological windows; "
        "block=%d rows/symbol",
        len(windows),
        test_block,
    )
    ...
    # -------------------------------------------------------------
    # Run every variant across every window
    # -------------------------------------------------------------
    ...
    log.info(
        "%-52s | Acc Mean | Acc SD | "
        "BUY Score Mean | BUY +Win | "
        "SELL Score Mean | SELL +Win",
        "Variant",
    )
    ...
    detail_path = (
        ARTIFACTS_DIR
        / "phase2c_feature_stability_detail.csv"
    )

    summary_path = (
        ARTIFACTS_DIR
        / "phase2c_feature_stability_summary.csv"
    )

    json_path = (
        ARTIFACTS_DIR
        / "phase2c_feature_stability.json"
    )

    detail = df.copy()

    detail["Selected_Features"] = (
        detail[
            "Selected_Features"
        ].apply(
            lambda x: ",".join(x)
    ...
    log.info(
        "PHASE 2C COMPLETE in %.1f minutes",
        (time.time() - t0) / 60,
    )
