...
def fetch_from_github(filename: str):
    if not GH_TOKEN or not GH_REPO: return None
    for path in [f"data/{filename}", filename]:
        try:
            # 🟢 FIX: Added a timestamp to the URL to bypass GitHub's aggressive CDN cache
            url = f"https://api.github.com/repos/{GH_REPO}/contents/{path}?ref={GH_BRANCH}&t={int(time.time())}"
            r = requests.get(url, headers=_gh_headers(), timeout=8)
            if r.status_code == 200:
                content = base64.b64decode(r.json()["content"]).decode("utf-8")
                if filename.endswith(".json"):
                    return json.loads(content)
                return content # Return raw string for bot.log
        except Exception:
            pass
    return None
...
def get_log_lines(n=200) -> str:
    # 🟢 FIX: The dashboard now pulls the log from GitHub instead of the empty Render server
    now = time.time()
    if "bot.log" in _cache and now - _cache_time.get("bot.log", 0) < CACHE_TTL:
        log_content = _cache["bot.log"]
    else:
        log_content = fetch_from_github("bot.log")
        if log_content:
            _cache["bot.log"] = log_content
            _cache_time["bot.log"] = now

    if log_content and isinstance(log_content, str):
        return "".join(log_content.splitlines(keepends=True)[-n:])
...
def api_open_trades():
    ...
    try:
        ...
        if r.ok:
            live_prices = {item["symbol"]: float(item["price"]) for item in r.json() if item["symbol"] in symbols}
    except Exception: pass
    ...
    for symbol, t in trades.items():
        ...
        tp2, progress = float(t.get("tp2", 0) or 0), 0.0
        if entry > 0 and tp2 > 0 and live > 0:
            total_dist = abs(tp2 - entry)
            moved_dist = abs(live - entry)
            if total_dist > 0: progress = min(100, max(0, moved_dist / total_dist * 100))

        result.append({
            **t, "symbol": symbol, "live_price": round(live, 6), "unrealised": round(upnl, 4),
            "pnl_pct": round(pnl_pct, 2), "progress": round(progress, 1), "entry": entry,
            "stop": float(t.get("stop",  0) or 0), "tp1": float(t.get("tp1", 0) or 0),
            "tp2": float(t.get("tp2", 0) or 0), "confidence": t.get("confidence", 0),
            "score": t.get("score", 0), "reasons": t.get("reasons", []),
        })
    return jsonify(result)
...
@app.route("/api/log")
def api_log():
    force_refresh("bot.log") # 🟢 Force GitHub cache bust before fetching
    return jsonify({"log": get_log_lines(200), "lines": 200})

@app.route("/api/market")
def api_market():
    symbols = ["BTCUSDT","ETHUSDT","BNBUSDT","SOLUSDT","AVAXUSDT","XRPUSDT","LINKUSDT","NEARUSDT","DOTUSDT","ADAUSDT","INJUSDT","ARBUSDT","OPUSDT","UNIUSDT","AAVEUSDT","FETUSDT","RENDERUSDT","SEIUSDT","SUIUSDT","APTUSDT"]
    prices = {}
    try:
        ...
        if r.ok:
            for item in r.json():
                if item["symbol"] in symbols:
                    prices[item["symbol"]] = {"price": float(item.get("lastPrice", 0)), "change_24h": float(item.get("priceChangePercent", 0)), "volume_24h": float(item.get("quoteVolume", 0)), "high_24h": float(item.get("highPrice", 0)), "low_24h": float(item.get("lowPrice", 0))}
    except Exception as e: log.warning(f"Market data: {e}")
...
def api_scan():
    ...
    for workflow in ["crypto_bot.yml", "crypto_bot.yaml", "main.yml"]:
        try:
            r = requests.post(f"https://api.github.com/repos/{GH_REPO}/actions/workflows/{workflow}/dispatches", headers={**_gh_headers(), "Content-Type": "application/json"}, json={"ref": GH_BRANCH, "inputs": {"mode": "scan"}}, timeout=15)
            if r.status_code in (204, 200):
                log.info(f"✅ GitHub Actions triggered via {workflow}")
                for f in ["trades.json","balance.json","signals.json","trade_history.json","scan_mode.json", "bot.log"]: force_refresh(f)
                return jsonify({"status": "triggered", "workflow": workflow, "message": "Scan started — results appear in ~60 seconds"})
            else:
                err_msg = f"{workflow} failed: {r.status_code} - {r.text}"
                log.error(err_msg); errors.append(err_msg)
...
def api_performance():
    ...
    history = get_data("trade_history.json", [])
    real    = [h for h in history if h.get("signal") != "RECOVERED"]
    wins, losses = [h for h in real if (h.get("pnl") or 0) > 0], [h for h in real if (h.get("pnl") or 0) <= 0]
    ...
    for h in real:
        ...
        by_symbol[sym]["trades"] += 1; by_symbol[sym]["pnl"] += h.get("pnl", 0)
    ...
    return jsonify({"total_trades": len(real), "wins": len(wins), "losses": len(losses), "win_rate": round(len(wins)/len(real)*100,1) if real else 0, "total_pnl": round(tpnl, 4), "avg_win": round(avg_win, 4), "avg_loss": round(avg_loss, 4), "profit_factor": pf, "by_symbol": by_symbol, "daily_pnl": daily})

@app.route("/api/config")
def api_config():
    return jsonify({"max_open_trades": 3, "risk_per_trade_pct": 2.0, "atr_stop_mult": 1.5, "atr_tp1_mult": 2.0, "atr_tp2_mult": 3.0, "min_confidence_active": 50, "min_confidence_quiet": 55, "min_score_active": 1, "min_score_quiet": 2, "min_adx": 15, "scan_interval_min": 15, "symbols": 20, "exchange": "Deribit Testnet", "model_accuracy": 73.1})
...
def api_close_trade():
    ...
    force_refresh("trades.json")
    trades = get_data("trades.json", {})
    if symbol not in trades: return jsonify({"error": f"{symbol} not in open trades"}), 404

    trade = trades.pop(symbol)
    history = get_data("trade_history.json", [])
    history.append({**trade, "close_price": float(trade.get("entry", 0)), "pnl": 0.0, "closed_at": datetime.now(timezone.utc).isoformat(), "close_reason": "Manual close via dashboard"})

    for p in [Path("trades.json"), Path("data/trades.json")]:
        try: p.parent.mkdir(exist_ok=True); json.dump(trades, open(p,"w"), indent=2)
        except Exception: pass
    for p in [Path("trade_history.json"), Path("data/trade_history.json")]:
        try: p.parent.mkdir(exist_ok=True); json.dump(history, open(p,"w"), indent=2, default=str)
        except Exception: pass

    force_refresh("trades.json"); force_refresh("trade_history.json")
    return jsonify({"status": "removed", "symbol": symbol, "warning": "Also close position on Deribit testnet UI!"})

@app.route("/api/sync")
def api_sync():
    for f in ["trades.json","trade_history.json","signals.json","balance.json","scan_mode.json", "bot.log"]: force_refresh(f)
