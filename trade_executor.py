        ...
        try:
            ...
            if not trade.get("closed") and oids.get("stop_loss"):
                ...
                if mark_breached and sl_not_waiting and not sl_hit:
                    ...
                    try:
                        # FIX: cancel the resting SL order FIRST — Deribit rejects
                        # a new reduce_only order (Code:11030) if this position's
                        # original SL is still live, since two reduce-only orders
                        # on the same position could double-reduce past its actual
                        # size if both filled. This is why every mark-breach/
                        # Scenario-B close was failing with
                        # 'other_reject invalid_reduce_only_order'.
                        if oids.get("stop_loss"):
                            try: deribit.cancel_order(oids["stop_loss"])
                            except Exception: pass
                        close_side = "SELL" if signal == "BUY" else "BUY"
                ...
                if not sl_hit and sl_breached and sl_not_waiting:
                    ...
                    try:
                        # Same fix as mark-breach above — cancel the resting SL
                        # first, or Deribit rejects the close with Code:11030.
                        if oids.get("stop_loss"):
                            try: deribit.cancel_order(oids["stop_loss"])
                            except Exception: pass
                        close_side = "SELL" if signal == "BUY" else "BUY"
