# trade_executor.py — OPTIMIZED MASTER VERSION
# 1. Correlation filter (max 2 same-direction trades)
...
def clean_ghost_orders(ex):
    """Cancels orphaned limit orders on Binance to free locked balance."""
    trades = load_trades()
    valid_ids = []
    ...
    try:
        log.info("  🧹 Sweeping exchange for ghost orders...")
        ...
        if cancelled > 0:
            log.info(f"  ✅ Swept and cancelled {cancelled} ghost orders! Balance unlocked.")
    except Exception as e:
        log.warning(f"  Sweep failed: {e}")
...
def sync_trade_history(ex):
    ...
    except Exception as e: log.warning(f"  ⚠️ History sync failed: {e}")
...
def auto_recover_trades(ex):
    ...
    except Exception as e: log.warning(f"  ⚠️ Auto-recover failed (safe to ignore): {e}")
...
def check_open_trades(ex):
    ...
    for symbol, trade in list(trades.items()):
        ...
        try:
            ...
            # ── BREAKEVEN TRAILING STOP LOGIC (TP1) ──
            if not trade["tp1_hit"] and "tp1" in oids:
                try:
                    o = ex.fetch_order(oids["tp1"], symbol)
                    if o["status"] == "closed":
                        trade["tp1_hit"] = True
                        avg_price = float(o.get("average") or o.get("price") or trade["tp1"])
                        pnl = _pnl(trade, avg_price, "tp1")
                        log.info(f"  🎯 TP1 HIT {symbol} pnl={pnl:+.4f}")
                        _send_close_alert(symbol, "TP1 HIT 🎯", pnl, entry, avg_price, trade["opened_at"])
            ...
            if trade["tp1_hit"] and not trade["tp2_hit"] and "tp2" in oids:
                try:
                    ...
                    if o["status"] == "closed":
                        trade["tp2_hit"] = True
                        trade["closed"]  = True
                        avg_price = float(o.get("average") or o.get("price") or trade["tp2"])
                        pnl = _pnl(trade, avg_price, "tp2")
                        log.info(f"  ✅ TP2 HIT {symbol} pnl={pnl:+.4f}")
                        _send_close_alert(symbol, "✅ FULL WIN (TP2)", pnl, entry, avg_price, trade["opened_at"])
                        _record_close(trade, avg_price, pnl, "TP2 hit")
                        to_remove.append(symbol)
                except Exception: pass
            ...
            if not trade.get("closed") and "stop_loss" in oids:
                try:
                    ...
                    if o["status"] == "closed":
                        trade["closed"] = True
                        avg_price = float(o.get("average") or o.get("price") or trade["stop"])
                        pnl = _pnl(trade, avg_price, "sl")
                        log.info(f"  ❌ SL HIT {symbol} pnl={pnl:+.4f}")
                        _send_close_alert(symbol, "❌ STOPPED OUT", pnl, entry, avg_price, trade["opened_at"])
                        _record_close(trade, avg_price, pnl, "SL hit")
...
def _quality_score(row_entry, row_confirm, signal, confidence):
    ...
    if signal == "BUY" and rsi < 45: score += 1; reasons.append(f"RSI bullish zone")
    elif signal == "SELL" and rsi > 55: score += 1; reasons.append(f"RSI bearish zone")

    e20, e50 = float(row_entry.get("ema20", 0)), float(row_entry.get("ema50", 0))
    ...
    c20, c50 = float(row_confirm.get("ema20", 0)), float(row_confirm.get("ema50", 0))
...
def _send_open_alert(symbol, signal, confidence, score, entry, stop, tp1, tp2, qty, risk_usd, balance, reasons, risk_mult=1.0):
    ...
    _send(f"🤖 *TESTNET TRADE OPENED*\n{emoji} *{signal} — {symbol}* {stars}\n🎯 Conf: *{confidence:.1f}%* | Score: *{score}/6*{risk_n}\n⚡ *ENTRY:* `{entry}`\n🛑 *STOP:* `{stop}`\n🎯 *TP1:* `{tp1}`\n💰 *Pos:* `{round(qty*entry,2)} USDT`")
...
def run_diagnostic():
    pass # Kept standard for diagnostics if needed
