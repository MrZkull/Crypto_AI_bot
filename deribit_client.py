# deribit_client.py — DEFINITIVE FIX
#
# THE ONE BUG THAT CAUSED EVERYTHING:
#   _post() was wrapping requests in JSON-RPC 2.0 envelope:
#   {"jsonrpc":"2.0","method":...,"params":body}
#   This format is WEBSOCKET ONLY.
#   REST API /api/v2/private/buy needs the body DIRECTLY.
#   Result: every SL/TP order returned -32602 silently → $0 shown in dashboard.
...
class DeribitClient:
    ...
    def _post(self, path: str, body: dict) -> dict:
        self._ensure_auth()
        # FIX: Send body directly to the REST endpoint. No JSON-RPC wrapper.
    ...
    def place_market_order(self, symbol: str, side: str, amount) -> dict:
        ...
        # FIX: Clean POST to REST API without time_in_force parameter
    ...
    def test_connection(self) -> bool:
        try:
            ...
            log.info(f"✅ Deribit OK — ${total:.2f} | {len(TRADEABLE_SYMBOLS)} symbols | POST:✅ (Direct REST API)")
            return True
        except Exception as e:
...
class DeribitClient:
    ...
    def _post(self, path: str, body: dict) -> dict:
        """
        HTTP POST — sends body DIRECTLY as JSON to REST API.
        NO JSON-RPC wrapper. That format is for WebSocket only.
        {"jsonrpc":"2.0",...} on REST → -32602 Invalid params on every order.
        """
        self._ensure_auth()
        r    = self.session.post(f"{self.base}{path}", json=body, timeout=15)
        data = r.json()
    ...
    def place_limit_order(self, symbol: str, side: str, amount,
                          price: float, stop_price: float = None) -> dict:
        """
        Plain limit or stop-limit order.
        Uses trigger_price field (not stop_price) for stop-limit orders.
        reduce_only=True ensures these only close positions.
        """
        instrument = self.get_instrument_name(symbol)
        method     = "/private/buy" if side.upper() == "BUY" else "/private/sell"
        safe_price = self.round_price(symbol, price)
    ...
    def test_connection(self) -> bool:
        try:
            total = self.get_total_equity_usd()
            log.info(f"✅ Deribit OK — ${total:.2f} | {len(TRADEABLE_SYMBOLS)} symbols | POST:✅ (no JSON-RPC)")
            return True
        except Exception as e:
            log.error(f"✗ Deribit: {e}")
            raise
